...
| [`jimothy-android-1.0.19-gradle.apk`](dist/jimothy-android-1.0.19-gradle.apk) | Phone install with **closed-app alerts** (Gradle + NotificationScheduler) |
| [`jimothy-android-1.0.19-gradle.aab`](dist/jimothy-android-1.0.19-gradle.aab) | Play Console bundle (Gradle + closed-app alerts). Sign/verify with your upload keystore — see godot/README.md |
...
**Phone + closed-app alerts:** install the prebuilt [`dist/jimothy-android-1.0.19-gradle.apk`](dist/jimothy-android-1.0.19-gradle.apk) (Godot-on-phone exports usually miss the scheduler plugin). See [`godot/README.md`](godot/README.md). Web needs a user tap to allow notifications on phones. Preference is saved.
