...
1. Download [`dist/jimothy-android-1.0.25-gradle.apk`](../dist/jimothy-android-1.0.25-gradle.apk)
...
Prebuilt Gradle AAB (package **`com.jimothypet.app`**, versionCode **25**, closed-app alerts):

- [`dist/jimothy-android-1.0.25-gradle.aab`](../dist/jimothy-android-1.0.25-gradle.aab)
...
1. Upload **only** `jimothy-android-1.0.25-gradle.aab` (re-signed — see below). Do **not** upload 1.0.16–1.0.22.
2. In Play Console, open **every** track: Internal testing, Closed testing, Open testing, Production.
3. On each track that still has a version **≤1.0.22** / that still declares `USE_EXACT_ALARM`, **deactivate** it or **replace** it with 1.0.25.
4. Retry the review / policy check after all active artifacts are 1.0.25+.
...
cp jimothy-android-1.0.25-gradle.aab jimothy-play.aab
...
Custom permissions include `SCHEDULE_EXACT_ALARM` only (not `USE_EXACT_ALARM` — Play rejects that for non-calendar/alarm apps). If Play still flags `USE_EXACT_ALARM`, replace older builds on **all tracks** (see above).
