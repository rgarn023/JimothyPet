...
1. Download [`dist/jimothy-android-1.0.29-gradle.apk`](../dist/jimothy-android-1.0.29-gradle.apk)
...
- [`dist/jimothy-android-1.0.29-gradle.aab`](../dist/jimothy-android-1.0.29-gradle.aab)
...
1. Upload **only** `jimothy-android-1.0.29-gradle.aab` (re-signed — see below). Do **not** upload 1.0.16–1.0.22.
2. In Play Console, open **every** track: Internal testing, Closed testing, Open testing, Production.
3. On each track that still has a version **≤1.0.22** / that still declares `USE_EXACT_ALARM`, **deactivate** it or **replace** it with 1.0.29.
4. Retry the review / policy check after all active artifacts are 1.0.29+.
...
cp jimothy-android-1.0.29-gradle.aab jimothy-play.aab
