...
const JimothyNotify = (() => {
  ...
  function predictEvents(state) {
    ...
    const clampDelay = (d, min = AWAY) =>
      Math.max(min, Math.min(MAX_DELAY, Math.ceil(d)));

    if (state.stage === "bush") {
      out.push({
        key: "bush",
        delay: Math.max(15, Math.min(MAX_DELAY, Math.ceil(60 - age))),
        title: "Jimothy popped out of the bush",
        body: "Baby kit Jimothy burst from the leaves. Open the app!",
      });
      return out;
    ...
    if (state.sick) {
      flags.push("sick");
      lines.push("Sick — open Action → Heal.");
    }
    if (state.stubborn) {
      flags.push("acting up");
      lines.push(
    ...
    if (messCount > 0 || state.hasMess) {
      flags.push("waste");
      lines.push(messCount > 1 ? `Waste — ${messCount} piles. Clean them.` : "Waste — clean the nest.");
    }
    if (state.stage !== "baby" && state.energy >= 18 && state.happy < 25) {
      flags.push("bored");
      lines.push("Bored — open Play.");
    }
    if (flags.length) {
      out.push({
        key: "care",
        delay: AWAY,
        title: careTitle(flags),
        body: lines.join("\n"),
      });
    }
    ...
    if (state.stage !== "baby" && state.happy >= 25) {
      const bd = clampDelay((state.happy - 25) / HAPPY_RATE);
      if (!best || bd < best.delay) {
        best = {
          key: "bored",
          delay: bd,
  ...
  /** Closed-tab scheduling only — no shade spam while the page is open. */
